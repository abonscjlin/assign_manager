...
def direct_workforce_calculation():
    ...
    with redirect_stdout(output_buffer):
        ...
        print(f"   如全由資深技師處理: {senior_time_needed} 分鐘")
        print(f"   如全由一般技師處理: {junior_time_needed} 分鐘")
        ...
        if senior_workers_to_add == 0:
            ...
            print(f"   ✅ 建議增加: {senior_workers_to_add} 名資深技師")
            solutions.append({
                'type': '只增加資深技師',
                'senior_add': senior_workers_to_add,
                'junior_add': 0,
                'cost_factor': senior_workers_to_add * 1.5,  # 假設資深技師成本1.5倍
                'description': f'增加{senior_workers_to_add}名資深技師'
            })
        ...
        if junior_workers_to_add == 0:
            ...
            print(f"   ✅ 建議增加: {junior_workers_to_add} 名一般技師")
            solutions.append({
                'type': '只增加一般技師',
                'senior_add': 0,
                'junior_add': junior_workers_to_add,
                'cost_factor': junior_workers_to_add * 1.0,
                'description': f'增加{junior_workers_to_add}名一般技師'
            })
        ...
        # 按難度優先級分配剩餘時間（簡單工作給一般技師，複雜工作給資深技師）
        ...
        if remaining_gap <= 0:
            ...
            if best_mix:
                ...
                print(f"   ✅ 建議增加: {senior_add} 名資深技師 + {junior_add} 名一般技師")
                solutions.append({
                    'type': '混合增加',
                    'senior_add': senior_add,
                    'junior_add': junior_add,
                    'cost_factor': senior_add * 1.5 + junior_add * 1.0,
                    'description': f'增加{senior_add}資深+{junior_add}一般技師'
                })
            else:
        ...
        print(f"")
        print(f"   📈 **具體調整：**")
        print(f"   - 資深技師：{actual_senior_count} → {actual_senior_count + recommended['senior_add']} 人 (+{recommended['senior_add']}人)")
        print(f"   - 一般技師：{actual_junior_count} → {actual_junior_count + recommended['junior_add']} 人 (+{recommended['junior_add']}人)")
...
def save_to_file(content):
    ...
    try:
        ...
        print(f"✅ 人力需求分析報告已保存到: {output_file}")
